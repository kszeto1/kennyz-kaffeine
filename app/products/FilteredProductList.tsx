    ...
    <div className="container mx-auto px-4 min-h-screen">
      <div className="flex flex-wrap -mx-4">
        {filteredProducts.map((product: Product) => (
          <div key={product.id} className="w-full pt-10">
            {product.items.map((item) => (
              <div key={item.subCategory} className="mb-8">
                <h3 className="text-3xl font-bold mb-4 py-4">{item.subCategory}</h3>
                <div className="flex flex-wrap -mx-2">
                  {item.items.map((subItem) => (
                    <div key={subItem.name} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/5 px-2 mb-4 mr-3">
